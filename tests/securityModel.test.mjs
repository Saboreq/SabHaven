...
test('Storage upload return rows are readable only in the uploader reservation scope', async () => {
  const sql = await readFile(uploadReturnMigrationUrl, 'utf8');
  assert.match(sql, /create policy "Members read their reserved upload objects"/i);
  assert.match(sql, /on storage\.objects for select[\s\S]*to authenticated/i);
  assert.match(sql, /bucket_id = 'downloads'/i);
...
test('Storage policies resolve file reservations through a scoped definer helper', async () => {
  const sql = await readFile(storageReservationMigrationUrl, 'utf8');
  assert.match(sql, /create or replace function private\.has_file_upload_reservation/i);
  assert.match(sql, /security definer[\s\S]*set search_path = ''/i);
  assert.match(sql, /file\.storage_path = p_storage_path/i);
  assert.match(sql, /file\.owner_id = p_user_id/i);
  assert.match(sql, /revoke all on function private\.has_file_upload_reservation\(text, uuid\) from public/i);
  assert.match(sql, /grant execute on function private\.has_file_upload_reservation\(text, uuid\) to authenticated/i);
  assert.match(sql, /for insert[\s\S]*private\.has_file_upload_reservation\(name, \(select auth\.uid\(\)\)\)/i);
  assert.match(sql, /for select[\s\S]*private\.has_file_upload_reservation\(name, \(select auth\.uid\(\)\)\)/i);
});
